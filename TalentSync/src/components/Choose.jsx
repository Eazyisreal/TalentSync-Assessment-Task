...
export default function Choose() {
    return (
        <section className="flex flex-col py-[6rem] w-full lg:px-[2rem] px-4 gap-20 bg-white" >
            <div className="flex flex-col gap-5">
                <div className="flex flex-col gap-3">
                    <p className="text-blue700 font-inter text-[1.125rem] font-semibold">The ClearLink Advantage</p>
                    <h1 className="text-gray800 text-5xl font-semibold font-inter">Why choose ClearLink?</h1>
                </div>
                <p className="font-inter text-2xl w-[70%]">In a world where connection is everything, ClearLink takes the lead. Our cutting-edge video conferencing app offers:</p>
                <img className="absolute z-10  left-[61%]" src={handArrow} alt="" />
            </div>
                    ...
                    {textContainer.map((item) => (
                        <div key={item} className="flex flex-col gap-5">
                            ...
                            <div className="flex flex-col gap-2">
                                <h2 className="font-inter text-gray900 text-xl font-semibold">{item.title}</h2>
                                <p className="text-gray600  font-inter">{item.text}</p>
                            </div>
                        </div>
                    ))}
                </div>
